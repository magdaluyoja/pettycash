...
function FPDF($orientation='P',$unit='mm',$format='A4')
{
	...
	if(is_string($format))
	{
		...
		elseif($format=='invoice')
			$format=array(623,792);
		elseif($format=='legal')
			$format=array(612,1008);
		elseif($format=='checkvoucher')
			$format=array(618, 507.4);
		elseif($format=='big')
			$format=array(1030, 792);
		elseif($format=='usfanfold')
			$format=array(792,1071);//1008-
		elseif($format=='wide')
			$format=array(792,1500);//1008-
		elseif($format=='halfletter')
	...
	$this->PDFVersion='1.3';
	//maxwidth
	$this->maxwidth = ($this->wPt/$this->k)-$this->lMargin-$this->rMargin;

	if($this->header AND method_exists($this, 'GenerateLinear')) foreach(array_keys($this->header) as $y) if($this->header[$y]['w']{strlen($this->header[$y]['w'])-1} == '%') {
		$this->header[$y]['w'] = $this->header[$y]['w'] * ($this->maxwidth/100);
	}
}
...
function SetX($x)
{
	if($this->adjustX) $x += $this->adjustX;
	//Set x position
	if($x>=0)
		$this->x=$x;
	else
...
function SetY($y)
{
	if($this->adjustY) $y += $this->adjustY;
...
function _putpages()
{
	if(method_exists($this, 'PostRender')) $this->PostRender();
	$nb=$this->page;
